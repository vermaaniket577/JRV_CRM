  ...
  <div class="min-h-screen bg-slate-100 flex font-sans text-slate-900">
    ...
    <div class="flex-1 flex flex-col min-w-0">
      ...
      <div class="p-8 space-y-6 flex-1 overflow-y-auto max-w-7xl mx-auto w-full">
        ...
        <!-- QUICK-START BUSINESS LAUNCHPAD BANNER (Clean Standard Light Theme) -->
        <transition enter-active-class="transition duration-300 ease-out" enter-from-class="opacity-0 -translate-y-2" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition duration-200 ease-in" leave-from-class="opacity-100" leave-to-class="opacity-0">
          <div v-if="showLaunchpad" class="bg-white text-slate-900 rounded-3xl p-6 sm:p-7 shadow-xs border border-slate-200/90 relative overflow-hidden">
            <!-- Decorative subtle background accents -->
            <div class="absolute -right-16 -top-16 w-64 h-64 bg-red-500/5 rounded-full blur-3xl pointer-events-none"></div>
            <div class="absolute -left-16 -bottom-16 w-64 h-64 bg-indigo-500/5 rounded-full blur-3xl pointer-events-none"></div>
            ...
            <div class="relative z-10 space-y-5">
              ...
              <div class="flex flex-wrap items-center justify-between gap-3">
                ...
                <button @click="dismissLaunchpad" class="text-slate-400 hover:text-slate-700 p-1.5 rounded-lg hover:bg-slate-100 transition-all text-xs flex items-center gap-1 cursor-pointer font-medium">
                  <span>Dismiss</span>
              ...
              <div>
                ...
                <p class="text-xs sm:text-sm text-slate-500 mt-1 max-w-2xl leading-relaxed font-medium">
              ...
              <div class="bg-slate-50/90 border border-slate-200 rounded-2xl p-4 flex flex-wrap items-center justify-between gap-3 shadow-2xs">
                <div class="flex items-center gap-3">
                  <div class="w-10 h-10 rounded-xl bg-red-50 border border-red-200 text-red-600 flex items-center justify-center text-lg shadow-2xs shrink-0">
                    🌐
                  </div>
                  <div>
                    <div class="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Workspace Subdomain</div>
                    <div class="text-sm font-bold text-slate-900 font-mono flex items-center gap-2">
                      <span>{{ tenantSubdomainUrl }}</span>
                      <span class="text-xs font-extrabold px-2 py-0.5 rounded-md bg-emerald-50 text-emerald-700 border border-emerald-200">Online</span>
                    </div>
                  </div>
                </div>
                <div class="flex items-center gap-2">
                  <button 
                    ...
                    class="px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl flex items-center gap-1.5 transition-all shadow-xs cursor-pointer"
                    title="Upload and restore database or bulk import data"
                  >
                    <CircleStackIcon class="w-4 h-4 text-emerald-100" />
                    <span>Upload Database</span>
                  </button>
                  <button @click="copySubdomain" class="px-3.5 py-2 bg-white hover:bg-slate-100 text-slate-700 text-xs font-bold rounded-xl border border-slate-300 flex items-center gap-1.5 transition-all shadow-2xs cursor-pointer">
                    <CheckIcon v-if="subdomainCopied" class="w-4 h-4 text-emerald-600" />
                    <ClipboardDocumentIcon v-else class="w-4 h-4 text-slate-400" />
                    <span>{{ subdomainCopied ? 'Copied!' : 'Copy Subdomain' }}</span>
                  </button>
                  <a :href="tenantSubdomainUrl" target="_blank" rel="noopener noreferrer" class="px-3.5 py-2 bg-red-600 hover:bg-red-700 text-white text-xs font-bold rounded-xl flex items-center gap-1.5 transition-all shadow-xs cursor-pointer">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5 pt-1">
                ...
                <Link :href="primaryActionRoute" class="group bg-slate-50/70 hover:bg-white border border-slate-200 hover:border-red-300 rounded-2xl p-4 sm:p-5 transition-all duration-200 flex flex-col justify-between space-y-3 shadow-2xs hover:shadow-xs">
                  <div class="flex items-start justify-between">
                    <div class="w-9 h-9 rounded-xl bg-red-50 text-red-600 border border-red-100 flex items-center justify-center font-bold text-base group-hover:scale-110 transition-transform">
                      1️⃣
                    </div>
                    <span class="text-[10px] font-extrabold uppercase px-2 py-0.5 rounded bg-red-50 text-red-700 border border-red-200">Quick Start</span>
                  </div>
                  <div>
                    <div class="font-extrabold text-sm text-slate-900 group-hover:text-red-600 transition-colors">
                  ...
                  <div class="text-xs font-bold text-red-600 flex items-center gap-1 group-hover:translate-x-1 transition-transform">
                ...
                <Link href="/tenant/settings/integration" class="group bg-slate-50/70 hover:bg-white border border-slate-200 hover:border-indigo-300 rounded-2xl p-4 sm:p-5 transition-all duration-200 flex flex-col justify-between space-y-3 shadow-2xs hover:shadow-xs">
                  <div class="flex items-start justify-between">
                    <div class="w-9 h-9 rounded-xl bg-indigo-50 text-indigo-600 border border-indigo-100 flex items-center justify-center font-bold text-base group-hover:scale-110 transition-transform">
                      2️⃣
                    </div>
                    <span class="text-[10px] font-extrabold uppercase px-2 py-0.5 rounded bg-indigo-50 text-indigo-700 border border-indigo-200">Automate</span>
                  </div>
                  <div>
                  ...
                  <div class="text-xs font-bold text-indigo-600 flex items-center gap-1 group-hover:translate-x-1 transition-transform">
                ...
                <Link href="/tenant/settings/navigation" class="group bg-slate-50/70 hover:bg-white border border-slate-200 hover:border-amber-300 rounded-2xl p-4 sm:p-5 transition-all duration-200 flex flex-col justify-between space-y-3 shadow-2xs hover:shadow-xs">
                  <div class="flex items-start justify-between">
                    <div class="w-9 h-9 rounded-xl bg-amber-50 text-amber-600 border border-amber-100 flex items-center justify-center font-bold text-base group-hover:scale-110 transition-transform">
                      3️⃣
                    </div>
                    <span class="text-[10px] font-extrabold uppercase px-2 py-0.5 rounded bg-amber-50 text-amber-700 border border-amber-200">Customization</span>
                  </div>
                  <div>
                    <div class="font-extrabold text-sm text-slate-900 group-hover:text-amber-600 transition-colors">
                  ...
                  <div class="text-xs font-bold text-amber-600 flex items-center gap-1 group-hover:translate-x-1 transition-transform">
                    <span>Configure</span>
                    <ArrowRightIcon class="w-3.5 h-3.5" />
                  </div>
                </Link>

                <!-- Action 4: Upload & Restore Database -->
                <button 
                  type="button" 
                  @click="isUploadDbModalOpen = true" 
                  class="group bg-slate-50/70 hover:bg-white border border-slate-200 hover:border-emerald-300 rounded-2xl p-4 sm:p-5 transition-all duration-200 flex flex-col justify-between space-y-3 shadow-2xs hover:shadow-xs text-left cursor-pointer"
                >
                  <div class="flex items-start justify-between">
                    <div class="w-9 h-9 rounded-xl bg-emerald-50 text-emerald-600 border border-emerald-100 flex items-center justify-center font-bold text-base group-hover:scale-110 transition-transform">
                      4️⃣
                    </div>
                    <span class="text-[10px] font-extrabold uppercase px-2 py-0.5 rounded bg-emerald-50 text-emerald-700 border border-emerald-200">Data Import</span>
                  </div>
                  <div>
                    <div class="font-extrabold text-sm text-slate-900 group-hover:text-emerald-600 transition-colors">
                      Upload Database / Data
                    </div>
                    <p class="text-xs text-slate-500 mt-1 leading-relaxed">
                      Upload your .sql database dump or import Excel/CSV records directly into your CRM.
                    </p>
                  </div>
                  <div class="text-xs font-bold text-emerald-600 flex items-center gap-1 group-hover:translate-x-1 transition-transform">
                    <span>Upload Database</span>
                    <ArrowRightIcon class="w-3.5 h-3.5" />
                  </div>
                </button>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
          ...
          <div class="lg:col-span-8 bg-gradient-to-r from-red-100 via-rose-100 to-pink-100 border border-red-200 rounded-3xl p-8 flex flex-col justify-between shadow-xs relative overflow-hidden">
            ...
            <div class="pt-6 z-10 flex items-center gap-3">
              ...
              <button 
                type="button" 
                @click="isUploadDbModalOpen = true" 
                class="px-5 py-2.5 bg-white hover:bg-slate-100 text-slate-800 text-xs font-bold rounded-xl border border-slate-300 shadow-xs inline-flex items-center gap-2 transition-all cursor-pointer"
              >
          ...
          <!-- Light Hero Widget -->
          <div class="lg:col-span-4 bg-white border border-slate-200 text-slate-900 rounded-3xl p-8 flex flex-col justify-between shadow-xs relative overflow-hidden">
            <div class="space-y-2 z-10">
              <span class="text-xs font-extrabold text-indigo-600 uppercase tracking-widest">System Status</span>
              <h3 class="text-lg font-black text-slate-900">CRM Cloud Platform</h3>
              <p class="text-xs text-slate-500 leading-relaxed">Automate follow-ups, sync client communications, and monitor pipeline conversion rates in real time.</p>
            </div>
            <div class="pt-6 z-10 flex items-center gap-2 text-xs font-semibold text-slate-600">
