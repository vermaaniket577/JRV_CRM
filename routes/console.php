...
Artisan::command('lb:status', function (LoadBalancerService $service) {
    $this->info('====================================================');
    ...
    return 0;
})->purpose('Display JRV CRM Software Load Balancer cluster status, upstream nodes, and traffic metrics');

Artisan::command('lb:check', function (LoadBalancerService $service) {
    ...
    return 0;
})->purpose('Probe health and measure latency for all cluster nodes');

Artisan::command('lb:simulate {requests=100 : Number of requests to simulate} {--algorithm= : Balancing algorithm to test}', function (LoadBalancerService $service) {
    ...
    $this->table($headers, $rows);
    return 0;
})->purpose('Simulate load balancer traffic distribution across healthy nodes');
